        ...
        <main class="flex-grow container mx-auto px-4 py-8">
            ...
            <div class="mb-6">
                <div class="flex justify-between items-center mb-4">
                    <h2 class="text-2xl font-bold text-gray-700">Gestion des Niveaux d'Accès</h2>
                ...
                <div class="relative mb-4">
                    <input type="text" id="searchInput" placeholder="Rechercher un niveau d'accès..." 
            ...
            <div class="bg-white rounded-xl shadow-lg p-6 md:p-8 mb-8 transform transition-all duration-300 hover:shadow-xl">
                <h3 class="text-xl font-semibold text-gray-700 mb-6 border-b pb-3">
                    <?= isset($_GET['id_niveau_acces']) ? "Modifier le niveau d'accès" : "Ajouter un nouveau niveau d'accès" ?>
                </h3>

                <form method="POST" action="?page=parametres_generaux&action=niveaux_acces" id="niveauForm" class="space-y-6">
                    <?php if($niveau_a_modifier): ?>
                        <input type="hidden" name="id_niveau_acces" value="<?= htmlspecialchars($niveau_a_modifier->id_niveau_acces_donnees) ?>">
                    <?php endif ?>
                    
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <!-- Libellé Niveau -->
                        <div class="form-group">
                            <label for="lib_niveau_acces" class="block text-sm font-medium text-gray-700 mb-1">
                                Libellé du niveau d'accès <span class="text-red-500">*</span>
                            </label>
                            <input type="text" id="lib_niveau_acces" name="niveau_acces_donnees" required
                            ...
                            <div class="text-red-500 text-sm mt-1 hidden" id="lib_niveau_acces-error"></div>
                        </div>
                    </div>
                    ...
                    <div class="flex justify-start space-x-3 mt-6">
                        <?php if (isset($_GET['id_niveau_acces'])): ?>
                            <button type="submit" name="btn_add_niveau_acces_donnees"
                                ...
                                <i class="fas fa-save mr-2"></i> Modifier le niveau d'accès
                            </button>
                        ...
                        <?php else: ?>
                            <button type="submit" name="btn_add_niveau_acces_donnees"
                                ...
                                <i class="fas fa-plus mr-2"></i> Ajouter le niveau d'accès
                            </button>
                        <?php endif; ?>
                    </div>
                </form>
                                ...
                                <?php if (!empty($listeNiveaux)) : ?>
                                    <?php foreach ($listeNiveaux as $niveau) : ?>
                                        <tr class="hover:bg-gray-50 transition-colors">
                                            ...
                                            <td class="px-4 py-3 text-sm text-gray-900 font-medium"><?= htmlspecialchars($niveau->id_niveau_acces_donnees) ?></td>
                                            <td class="px-4 py-3 text-sm text-gray-700"><?= htmlspecialchars($niveau->lib_niveau_acces_donnees) ?></td>
                                            <td class="px-4 py-3 text-center">
                                                <div class="flex justify-center space-x-2">
                                                    ...
                                                    <button type="button" 
                                                            ...
                                                            data-id="<?= $niveau->id_niveau_acces_donnees ?>">
                                ...
                                <?php else : ?>
                                    <tr>
                                        <td colspan="4" class="text-center text-sm text-gray-500 py-4">
                                            Aucun niveau d'accès enregistré.
                                        </td>
                                    </tr>
                                <?php endif; ?>
                            </tbody>
