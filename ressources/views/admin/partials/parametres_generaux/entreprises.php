<?php
$entreprises_a_modifier = $GLOBALS['entreprise_a_modifier'] ?? null;
?>
<!DOCTYPE html>
<html lang="fr">
...
<body class="bg-gray-100">
        ...
        <main class="flex-grow container mx-auto px-4 py-8">
            ...
            <div class="mb-6">
                <div class="flex justify-between items-center mb-4">
                    <h2 class="text-2xl font-bold text-gray-700">Gestion des Entreprises</h2>
            ...
            <div class="bg-white rounded-xl shadow-lg p-6 md:p-8 mb-8 transform transition-all duration-300 hover:shadow-xl">
                ...
                <form method="POST" action="?page=parametres_generaux&action=entreprises" id="entrepriseForm" class="space-y-6">
                    <?php if($entreprises_a_modifier): ?>
                        <input type="hidden" name="id_entreprise" value="<?= htmlspecialchars($entreprises_a_modifier->id_entreprise) ?>">
                    <?php endif ?>
                    
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <!-- Libellé Entreprise -->
                        <div class="form-group">
                            <label for="lib_entreprise" class="block text-sm font-medium text-gray-700 mb-1">
                                Nom de l'entreprise <span class="text-red-500">*</span>
                            </label>
                            <input type="text" id="lib_entreprise" name="entreprise" required
                                   value="<?= $entreprises_a_modifier ? htmlspecialchars($entreprises_a_modifier->lib_entreprise) : '' ?>"
                            ...
                            <div class="text-red-500 text-sm mt-1 hidden" id="lib_entreprise-error"></div>
                        </div>
                    </div>
                    ...
                    <div class="flex justify-start space-x-3 mt-6">
                        <?php if (isset($_GET['id_entreprise'])): ?>
                            <button type="submit" name="btn_add_entreprise"
                                ...
                                <i class="fas fa-save mr-2"></i> Modifier l'entreprise
                            </button>
            ...
            <div class="bg-white rounded-xl shadow-lg overflow-hidden">
                <div class="p-6">
                    <div class="overflow-x-auto">
                        <table id="entrepriseTable" class="w-full">
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                <?php $listeEntreprises = $GLOBALS['listeEntreprises'] ?? []; ?>
                                <?php if (!empty($listeEntreprises)) : ?>
                                    <?php foreach ($listeEntreprises as $entreprise) : ?>
                                        <tr class="hover:bg-gray-50 transition-colors">
                                            <td class="px-4 py-3 text-center">
                                                <input type="checkbox" name="selected_ids[]" value="<?= htmlspecialchars($entreprise->id_entreprise) ?>"
                                                       class="row-checkbox form-checkbox h-4 w-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500">
                                            </td>
                                            <td class="px-4 py-3 text-sm text-gray-900 font-medium"><?= htmlspecialchars($entreprise->id_entreprise) ?></td>
                                            <td class="px-4 py-3 text-sm text-gray-700"><?= htmlspecialchars($entreprise->lib_entreprise) ?></td>
                                            <td class="px-4 py-3 text-center">
                                                <div class="flex justify-center space-x-2">
                                                    <a href="?page=parametres_generaux&action=entreprises&id_entreprise=<?= $entreprise->id_entreprise ?>"
    ...
    <script>
        $(document).ready(function() {
            ...
            const table = $('#entrepriseTable').DataTable({
                dom: 'Bfrtip',
                buttons: [
                    {
                        extend: 'excel',
