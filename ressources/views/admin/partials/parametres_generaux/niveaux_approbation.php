        ...
        <main class="flex-grow container mx-auto px-4 py-8">
            ...
            <div class="mb-6">
                ...
                <div class="relative mb-4">
                    <input type="text" id="searchInput" placeholder="Rechercher un niveau d'approbation..." 
            ...
            <div class="bg-white rounded-xl shadow-lg p-6 md:p-8 mb-8 transform transition-all duration-300 hover:shadow-xl">
                <h3 class="text-xl font-semibold text-gray-700 mb-6 border-b pb-3">
                    <?= isset($_GET['id_niv_approb']) ? "Modifier le niveau d'approbation" : "Ajouter un nouveau niveau d'approbation" ?>
                </h3>

                <form method="POST" action="?page=parametres_generaux&action=niveaux_approbation" id="niveauForm" class="space-y-6">
                    <?php if($niveau_a_modifier): ?>
                        <input type="hidden" name="id_niv_approb" value="<?= htmlspecialchars($niveau_a_modifier->id_niv_approb) ?>">
                    <?php endif ?>
                    
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <!-- Libellé Niveau -->
                        <div class="form-group">
                            <label for="lib_niv_approb" class="block text-sm font-medium text-gray-700 mb-1">
                                Libellé du niveau <span class="text-red-500">*</span>
                            </label>
                            <input type="text" id="lib_niv_approb" name="lib_niv_approb" required
                                   value="<?= $niveau_a_modifier ? htmlspecialchars($niveau_a_modifier->lib_niv_approb) : '' ?>"
                            ...
                            <div class="text-red-500 text-sm mt-1 hidden" id="lib_niv_approb-error"></div>
                        </div>
                    </div>

                    <div class="flex justify-start space-x-3 mt-6">
                        <?php if (isset($_GET['id_niv_approb'])): ?>
                                    ...
                                    <?php foreach ($listeNiveaux as $niveau) : ?>
                                        <tr class="hover:bg-gray-50 transition-colors">
                                            <td class="px-4 py-3 text-center">
                                                <input type="checkbox" name="selected_ids[]" value="<?= htmlspecialchars($niveau->id_niv_approb) ?>"
                                                       class="row-checkbox form-checkbox h-4 w-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500">
                                            </td>
                                            <td class="px-4 py-3 text-sm text-gray-900 font-medium"><?= htmlspecialchars($niveau->id_niv_approb) ?></td>
                                            <td class="px-4 py-3 text-sm text-gray-700"><?= htmlspecialchars($niveau->lib_niv_approb) ?></td>
                                            <td class="px-4 py-3 text-center">
                                                <div class="flex justify-center space-x-2">
                                                    <a href="?page=parametres_generaux&action=niveaux_approbation&id_niv_approb=<?= $niveau->id_niv_approb ?>"
                                                       class="text-blue-500 hover:text-blue-700 transition-colors">
                                                        <i class="fas fa-edit"></i>
                                                    </a>
                                                    <button type="button" 
                                                            class="text-red-500 hover:text-red-700 transition-colors delete-btn"
                                                            data-id="<?= $niveau->id_niv_approb ?>">
