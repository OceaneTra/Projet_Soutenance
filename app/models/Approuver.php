...
class Approuver {
    ...
    /**
     * Ajoute une approbation ou désapprobation dans la table approuver
     */
    public function approuverRapport($id_rapport, $id_pers_admin, $id_approb, $decision, $commentaire_approv) {
        $stmt = $this->pdo->prepare("INSERT INTO approuver (id_rapport, id_pers_admin, id_approb, decision, date_approv, commentaire_approv) VALUES (?, ?, ?, ?, NOW(), ?)");
        return $stmt->execute([$id_rapport, $id_pers_admin, $id_approb, $decision, $commentaire_approv]);
    }
} 