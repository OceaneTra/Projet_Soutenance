...
class AuthController {
    ...
    public function login($login, $password) {
        ...
        if ($infoUtilisateur) {
            ...
            if($type_utilisateur === 'Etudiant'){
                // Récupérer les informations de l'étudiant via l'email
                $etudiant = $this->getEtudiantByEmail($infoUtilisateur['login_utilisateur']);
                if($etudiant) {
                    $_SESSION['num_etu'] = $etudiant->num_etu;
                    $_SESSION['nom_etu'] = $etudiant->nom_etu;
                    $_SESSION['prenom_etu'] = $etudiant->prenom_etu;
                    $_SESSION['email_etu'] = $etudiant->email_etu;
                    $_SESSION['promotion_etu'] = $etudiant->promotion_etu;
                    $_SESSION['date_naiss_etu'] = $etudiant->date_naiss_etu;
                    $_SESSION['genre_etu'] = $etudiant->genre_etu;
                }
            }
            else if($type_utilisateur === 'Enseignant simple' || $type_utilisateur === 'Enseignant administratif'){
    ...
    private function getEtudiantByEmail($email) {
        try {
            $query = "SELECT * FROM etudiants WHERE email_etu = :email";
